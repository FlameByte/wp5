<?php
if(!class_exists('DaisystorePosts_Widget')){
 class DaisystorePosts_Widget extends WP_Widget {
    
	public function __construct() {
		
		parent::__construct(
			'daisy_store_recent_posts', // Base ID
			__( 'Daisy Store: Recent Posts', 'daisy-store-companion' ), // Name
			array( 'description' => __( 'Recent Posts.', 'daisy-store-companion' ), ) // Args
		);
		
	}
 	function form( $instance ) {
 	    $defaults = array('list_num' => '4','style' => '1', 'title' => __( 'Recent Posts', 'daisy-store-companion' ));
...
<p>
  <label for="<?php echo $this->get_field_id( 'title' ); ?>">
    <?php _e('Title', 'daisy-store-companion'); ?>
    :</label>
  <br />
...
<p>
  <label for="<?php echo $this->get_field_id( 'list_num' ); ?>">
    <?php _e('Recent Posts List Num', 'daisy-store-companion'); ?>
    :</label>
  <br />
...
<?php
	...
	function widget( $args, $instance ) {
 		extract( $args );
 	    $title    = apply_filters(__('Recent Posts', 'daisy-store-companion'), esc_attr($instance['title']) );
...
<div class="widget-project">
  <div class="row">
   ...
   if ( has_post_thumbnail() ) {
         $featured_image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'daisy-store-widget-post-image');
		 $source = get_site_url();
		 if($featured_image[0] !=""){
...
function daisy_store_companion_recent_posts(){
	
	register_widget('DaisystorePosts_Widget');
						
}
add_action( 'widgets_init', 'daisy_store_companion_recent_posts' );
